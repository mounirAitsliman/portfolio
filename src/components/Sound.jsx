...
const Sound = () => {
  ...
  useEffect(() => {
    ...
    if (
      consent &&
      consentTime &&
      new Date(consentTime) + 3 * 24 * 60 * 60 * 1000 > new Date()
    ) {
  ...
  return (
    <div className="fixed top-4 right-2.5 xs:right-4 z-50 group">
      ...
      <motion.button
        ...
        className="w-10 h-10 xs:w-14 xs:h-14 text-foreground rounded-full flex items-center justify-center cursor-pointer z-50 p-2.5 xs:p-4 cusom-bg"
        aria-label={"home"}
        name={"home"}
      >
        {isPlaying ? (
